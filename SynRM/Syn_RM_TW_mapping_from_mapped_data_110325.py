...
calculate optimal points for given SynRM.
...
for Var in Dict_Map.keys():
    ...
    if "[mH]" in Var:
        fun_send_DataSet_2_Mtlb(Name_mat+"_mat", Dict_Map[Var].iloc[1:, 1:]*1e-3);
    else:
...
eng.eval(
    ...
    'beq = [];'+
    'lb=[0*I_max, 0*I_max];'+
    'ub=[1.2*I_max, 1.2*I_max];'+
    'x0=[-10; 10];'+
    'fun = @(x)fun_max_Trq(x, fun_L_d, fun_L_q, N_pp);'+
...
for T_ref in List_Trq:
    ...
    MWS["L_q"] = L_q;
    eng.eval(
        'fun_omega0=@(x)(R_s*I_d-x*I_q*L_q)^2+(R_s*I_q+x*I_d*L_d)^2-V_dc^2/3*0.95;'+
        'x0_f=N_pp*5000/30*pi;'
        , nargout=0
        );
    omega0 = eng.fsolve(MWS['fun_omega0'], MWS['x0_f'], options_f);
    n_r = omega0/N_pp/(2*pi)*60;
    for key, value in Dict_OP.items():
        if key != "U_s" and value != "[mH]":
            Dict_Bndry["MTPA:"+key+value].append(globals()[key]);
        elif value == "[mH]":
            Dict_Bndry["MTPA:"+key+value].append(globals()[key]*1e3);
    eng.eval(
        'x0=['+str(I_d)+'; '+str(I_q)+'];'
        , nargout=0
        );
...
plt.figure(figsize=(8, 6), dpi=500);
plt.plot(
    Dict_Bndry["MTPA:I_s[A]"], Dict_Bndry["MTPA:L_d[mH]"]
    );
plt.xlabel("$I_s$[A]", fontsize=14);
plt.ylabel("$L_d$[mH]", fontsize=14);
plt.grid();

plt.figure(figsize=(8, 6), dpi=500);
plt.plot(
    Dict_Bndry["MTPA:I_s[A]"], Dict_Bndry["MTPA:L_q[mH]"]
    );
plt.xlabel("$I_s$[A]", fontsize=14);
plt.ylabel("$L_q$[mH]", fontsize=14);
plt.grid();
...
for n_r in List_n_r:
    if n_r <= n_base:
        ...
        eng.eval(
            ...
            'x, omega0, fun_L_d, fun_L_q, R_s, U_max, I_max'+
            ');'
            , nargout=0
            );
        I_dq = eng.fmincon(
...
for idx_nr, n_r_ref in enumerate(List_n_r):
    ...
    for idx_Trq, Trq_ref in enumerate(List_Trq):
        ...
        if n_r_ref <= n_r_max_MTPA:
            ...
            I_d_max = Dict_Bndry["FW&MTPV:I_d[A]"][
                Dict_Bndry["FW&MTPV:n_r[rpm]"] == n_r_ref
                ];
            I_q_max = Dict_Bndry["FW&MTPV:I_q[A]"][
                Dict_Bndry["FW&MTPV:n_r[rpm]"] == n_r_ref
                ];
            MWS["T_ref"] = Trq_ref;
            eng.eval(
                'ub=['+str(I_d_max)+', '+str(I_q_max)+'];'+
                'nonlcon = @(x)fun_constr_Umax_equal_Tref('+
                'x, fun_L_d, fun_L_q, N_pp,'+
...
M2D = ansys.aedt.core.Maxwell2d(
    project=Path_AEDT+project_name+".aedt",
    design=design_name
    );
